function OUT = SPEC_P_episode_crown(LFPvals, sFreq, fq_range ,window_s, percentile_thresh, wavelet_width)
...
for f = 1:length(fq_range)
    [~, power, ~] = waveletdecomp(fq_range(f),LFPvals,sFreq,wavelet_width);
    ...
    second_ixis = round(1:(sFreq*window_s):length(power));
    ...
    if PLOT_IT == 1;
        ...
        title(['P episodes of ' num2str(fq_range(f)) 'Hz'])
    end
