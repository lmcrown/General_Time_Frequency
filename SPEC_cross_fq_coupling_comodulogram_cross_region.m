function [cmpac] = SPEC_cross_fq_coupling_comodulogram_cross_region(values_lf,values_hf,sFreq,lf_range,hf_range,num_iter,GOODIX,USE_GPU)
...
if length(values_lf)*length(lf_range) * length(hf_range) < 10e8 % A guess based on a limited sample
...
if ALL_AT_ONCE
    ...
    [LF_phase] = SPEC_waveletdecomp(lf_range,values_lf,sFreq,wavelet_size,USE_GPU);
    [~,HF_power] = SPEC_waveletdecomp(hf_range,values_hf,sFreq,wavelet_size,USE_GPU);
