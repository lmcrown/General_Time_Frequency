function[LFP] = Matt_Manual_Artifact_Reject(LFP,window_size_sec,overwrite)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if any(diff(LFP.timestamps)<0)
    disp('Timestamps are incorrect, were donwsampled at some point. Making up new ones.');
    LFP.timestamps = linspace(LFP.timestamps(1),LFP.timestamps(end),length(LFP.values));
end
TS = LFP.timestamps-LFP.timestamps(1);
values = LFP.values/10e3;
...
point_jump = round(window_size_sec*LFP.Fs);

ixis = [1:point_jump:length(LFP.values) length(LFP.values)];
interf = figure;
i = 2;
figure(interf)
...
set(c,'callback',{@exit,interf})
    function define_artifact(hb, ~, interf)
        ...
        if button == 3;
            ...
            LFP.bad_intervals(LFP.bad_intervals > length(LFP.values)) = length(LFP.values);
        else
        end
        for i_interval = 1:size(LFP.bad_intervals,1)
            values(...
